...
public class ApiRegionBiz {
	...
	public Result<RegionListDTO> listForProvinceLevel(UserRegionProvinceBO userRegionProvinceBO) {
		List<Region> list = regionDao.listByProvinceIdLevel(userRegionProvinceBO.getProvinceName(),userRegionProvinceBO.getLevel());
		if (CollectionUtil.isNotEmpty(list)) {
			RegionListDTO data = new RegionListDTO();
			data.setRegionList(PageUtil.copyList(list, RegionDTO.class));
			return Result.success(data);

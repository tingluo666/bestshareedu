...
import com.roncoo.education.user.service.common.bo.auth.UserInfoUpdateBO;
...
//			result = true;
	...
	private User register(String mobile, String password, String clientId) {
		...
		//教学意向
		LecturerIntention lecturerIntention = new LecturerIntention();
		lecturerIntention.setUserNo(user.getUserNo());
		lecturerIntention.setMobile(user.getMobile());
		lecturerIntentionDao.save(lecturerIntention);
...
//	public Result<Integer> updateInfo(UserInfoUpdateBO userInfoUpdateBO) {
//
//		Platform platform = platformDao.getByClientId(userInfoUpdateBO.getClientId());
//		if (null == platform) {
//			return Result.error("该平台不存在");
//		}
//		if (!StatusIdEnum.YES.getCode().equals(platform.getStatusId())) {
